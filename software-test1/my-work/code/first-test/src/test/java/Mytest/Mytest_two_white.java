...
public class Mytest_two_white {

//    private FrameFixture window;
//    @BeforeClass
//    public void setUp() throws IOException {
//        window=new FrameFixture(String.valueOf(new caculate_window()));
//    }

    @DataProvider
    public Object[][] providerMethod() throws IOException {
        String excelPath = "国际航线白盒.xlsx";
    ...
    public void test_out_china(String rowid, String main_context) throws IOException {
//        window.button("开始").click();
        String[] context=main_context.split(",");
        int i=Integer.parseInt(rowid);
        String class_kind=context[2];//舱类型
        String area=context[3];
        double ticket_price=2000.0;//票价
        String passenge_kind=context[1];//乘客类型
        double package_length=Double.parseDouble(context[4]);//长
        double package_width=Double.parseDouble(context[5]);//宽
        double package_height=Double.parseDouble(context[6]);//高
        double package_weight=Double.parseDouble(context[7]);//重量

        Ticket newticket=new Ticket("国际航线",area,class_kind,ticket_price,passenge_kind);
        ...
        cacul.out_china (passenger_Package.get(0),(int)Double.parseDouble(context[0]));

        Assert.assertEquals(cacul.to_all_tran_price(),Double.parseDouble(context[9]),main_context+"  计算结果和预期不符");
